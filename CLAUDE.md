...
evals/results/           recorded dogfood runs (config, results TSV, verdict, REPORT.md with a Friction section); never edit a results file
...
- Run one trial the way Phase 3 does, against the tiny fixture eval: `cd tests/fixtures/trialproj && python3 ../../../scripts/run_trial.py --config loop_config.json` — prints one JSON line with `primary`, `counters`, `timed_out`, and a `tail`. Set `AUTOLOOP_FIXTURE_MODE=sleep|crash|gate|fail` to see the other outcomes. (The fixture extracts with its own `extract.py` and `emit.py` helpers rather than `grep`, and `tests/check.py` substitutes the running interpreter, so the same fixture works on Windows.)
...
- `scripts/run_trial.py` runs `eval_command` under `trial_timeout_seconds` (killing the whole process tree on timeout, `taskkill /T` on Windows), applies every `extract` pattern, and prints one JSON line with the primary, the counters, and a bounded `tail` for crash diagnosis. `ok` is false when the eval exited non-zero or the primary did not extract. An extract command must exit 0 and print exactly one line (two lines are ambiguous, and an artifact printing its own metric line is the classic way to game a benchmark); its stderr is dropped; nan and inf are unreadable. A missing or invalid timeout falls back to 600 s rather than running unbounded.
- `scripts/adjudicate.py` takes one round's trial JSON lines and prints the exact results rows: `crash` (timed out / did not extract), `gate_fail` (any counter-metric violation, or a counter that did not extract — an unevaluable gate is not a passed gate), then `keep` for the best survivor only if it beats best-so-far by the noise floor, `max(min_delta, min_delta_pct% of best-so-far)` (strictly better at a floor of 0), `discard` for the rest. Round 0 is the baseline and produces no row (and a reason) if it crashes or already violates a gate. `--append` writes the rows into the results file so the agent never touches it. A discard that beat best-so-far but lost to a sibling is prefixed `lost to <id>` (and `inside the noise floor` when the gap was below it) so the candidate generator does not read it as a dead end. It imports `check_stop.py` for the gate and noise-floor rules and for best-so-far, so the two cannot disagree.
- `scripts/check_stop.py` decides stop/continue, and audits the log it reads: a `keep` row with no parseable primary, a second keep in a round, a gate violation, or a gain below the noise floor is listed in `warnings` and treated as no keep. The floor is relative on purpose (`min_delta_pct`): the first dogfood run grounded an absolute 90 ms floor at a 1319 ms baseline and then discarded a 16.9 ms candidate against a 73 ms best, because nothing could ever clear 90 ms again; an absolute floor that blocks every keep on a `min` metric is now warned about. That can only end a run earlier (an unearned keep would reset patience), which is the same argument that let `target` in, with one exception worth knowing: a bogus keep that happened to satisfy `target` used to stop the run and now does not. Two keeps in one round invalidate both; a keep missing a value for a gated counter is invalid (adjudicate.py applies the same rule, so an unevaluable gate is never a passed gate). A primary without a direction is refused with `stop: true` rather than defaulting to `min`; any other unusable config value falls back to its default with a warning, and an internal error prints `stop: true` with the error rather than a traceback, so the loop is never left to decide for itself. `max_rounds` is also enforced in candidate rows (`max_rounds x candidates_per_round`) because a reused round label is invisible to the round-based conditions; adjudicate.py refuses to write into a round that already has rows.
