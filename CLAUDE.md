...
- `scripts/adjudicate.py` takes one round's trial JSON lines and prints the exact results rows: `crash` (timed out / did not extract), `gate_fail` (any counter-metric violation, or a counter that did not extract — an unevaluable gate is not a passed gate), then `keep` for the best survivor only if it beats best-so-far by `min_delta` (strictly better at `min_delta` 0), `discard` for the rest. Round 0 is the baseline and produces no row (and a reason) if it crashes or already violates a gate. It imports `check_stop.py` for the gate and min_delta rules and for best-so-far, so the two cannot disagree.
- `scripts/check_stop.py` decides stop/continue, and audits the log it reads: a `keep` row with no parseable primary, a second keep in a round, a gate violation, or a gain below `min_delta` is listed in `warnings` and treated as no keep. That can only end a run earlier (an unearned keep would reset patience), which is the same argument that let `target` in. A primary without a direction is refused with `stop: true` rather than defaulting to `min`.
...
- `epsilon: null` (the template default) means 0.5% of the baseline value, derived from the baseline row and reported as `stats.epsilon_effective`. The old template value of 0.001 in metric units could never fire: with `patience` 8 below the 10-round window and any `min_delta` >= 0.001, a window gain under 0.001 needs zero keeps, which patience catches first. An explicit number is used as given; 0 disables.
